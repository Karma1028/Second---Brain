import { Sun, ListTodo, MoreHorizontal, Circle, CheckCircle2, HeartPulse, TrendingUp, BookOpen, Droplets, Link as LinkIcon, FileText, Image as ImageIcon, Plus, Database, Sparkles, Flame, Book, Smile } from 'lucide-react';
  ...
  return (
    <div className="flex-1 overflow-y-auto flex flex-col">
      ...
      <div className="px-8 pb-12 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Habit Streaks */}
          <div className="lg:col-span-2 rounded-xl border border-surface-border bg-surface-dark p-6 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <h3 className="text-foreground text-lg font-bold flex items-center gap-2 font-display">
                <Flame className="w-5 h-5 text-accent-orange" />
                Habit Streaks
              </h3>
              <a className="text-sm text-primary hover:text-foreground transition-colors" href="/habits">View Grid</a>
            </div>
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between p-3 rounded-lg bg-background-dark border border-surface-border">
                <span className="text-foreground text-sm font-medium">Deep Work (2h)</span>
                <div className="flex items-center gap-4">
                  <div className="flex gap-1">
                    {[1, 1, 0, 1, 1, 1, 1].map((done, i) => (
                      <div key={i} className={`w-3 h-3 rounded-sm ${done ? 'bg-primary' : 'bg-surface-border'}`}></div>
                    ))}
                  </div>
                  <span className="text-xs font-bold text-text-secondary w-8 text-right">12🔥</span>
                </div>
              </div>
              <div className="flex items-center justify-between p-3 rounded-lg bg-background-dark border border-surface-border">
                <span className="text-foreground text-sm font-medium">Meditation (10m)</span>
                <div className="flex items-center gap-4">
                  <div className="flex gap-1">
                  ...
                  <span className="text-xs font-bold text-text-secondary w-8 text-right">14🔥</span>
                </div>
              </div>
              <div className="flex items-center justify-between p-3 rounded-lg bg-background-dark border border-surface-border">
                <span className="text-foreground text-sm font-medium">Workout</span>
                <div className="flex items-center gap-4">
                  <div className="flex gap-1">
                    {[0, 1, 0, 1, 0, 1, 0].map((done, i) => (
                      <div key={i} className={`w-3 h-3 rounded-sm ${done ? 'bg-primary' : 'bg-surface-border'}`}></div>
                    ))}
                  </div>
                  <span className="text-xs font-bold text-text-secondary w-8 text-right">3🔥</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-surface-border bg-surface-dark p-6 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <a className="text-sm text-primary hover:text-foreground transition-colors" href="/journal">Write</a>
            </div>
            <div className="p-4 rounded-lg bg-background-dark border border-surface-border relative overflow-hidden group hover:border-primary/50 transition-colors cursor-pointer flex-1 flex flex-col">
              <div className="absolute top-0 left-0 w-1 h-full bg-primary"></div>
              <div className="flex justify-between items-start mb-2">
                <span className="text-xs font-medium text-text-secondary">Today, 08:30 AM</span>
                <Smile className="w-4 h-4 text-green-400" />
              </div>
              <h4 className="text-foreground font-medium mb-2">Morning Clarity</h4>
              <p className="text-text-secondary text-sm line-clamp-3">
                Woke up feeling refreshed. The new sleep schedule is definitely paying off. I need to make sure I maintain this momentum into the weekend. Planning to focus heavily on the backend architecture today.
              </p>
            </div>
          </div>
        </div>
